...
class PointsService {
  ...
  async updatePointsForConfirmedOrders(): Promise<{
    ...
    try {
      ...
      console.log(
        `📊 Found ${confirmedOrders.length} confirmed orders to process`
      );

      // Log chi tiết các order để debug
      for (const order of confirmedOrders) {
        console.log(
          `📋 Order ${order._id}: ${order.seats?.length || 0} seats, ${
      ...
      for (const order of confirmedOrders) {
        try {
          ...
          if (order.seats && order.seats.length > 0) {
            ...
            console.log(
              `  🎫 Seats: ${order.seats.length} x 5 = ${seatPoints} points`
            );
          }
          ...
          if (order.foodCombos && order.foodCombos.length > 0) {
            for (const combo of order.foodCombos) {
              ...
              if (
                comboData &&
                typeof comboData === "object" &&
                comboData.type === "combo"
              ) {
                ...
                console.log(
                  `  🍿 Combo (${comboData.name}): ${combo.quantity} x 5 = ${comboPoint} points`
                );
              } else {
                console.log(
                  `  🥤 Single item (${
                    comboData?.name || "Unknown"
                  }): 0 points (type: ${comboData?.type || "unknown"})`
                );
              }
            }
          }
          ...
          console.log(
            `  📊 Total points for order ${order._id}: ${pointsToAdd} (${seatPoints} seats + ${comboPoints} combos)`
          );
          ...
          if (pointsToAdd > 0) {
            ...
            console.log(
              `✅ Added ${pointsToAdd} points to user ${order.userId} for order ${order._id}`
            );
          }
      ...
      console.log(
        `✅ Points processing completed: ${totalPointsAdded} points added to ${updatedUsers.length} users`
      );
  ...
  async updatePointsForSingleOrder(orderId: string): Promise<{
    ...
    try {
      ...
      if (order.orderStatus !== "CONFIRMED") {
        console.log(
          `❌ Order ${orderId} status is not CONFIRMED: ${order.orderStatus}`
        );
        return {
          success: false,
          pointsAdded: 0,
      ...
      if (order.seats && order.seats.length > 0) {
        ...
        console.log(
          `  📍 Seats: ${order.seats.length} x 5 = ${
            order.seats.length * 5
          } points`
        );
      }
      ...
      if (order.foodCombos && order.foodCombos.length > 0) {
        for (const combo of order.foodCombos) {
          ...
          if (
            ...
            console.log(
              `  🍿 Combo (${comboData.name}): ${combo.quantity} x 5 = ${comboPoints} points`
            );
          } else {
            console.log(
              `  🥤 Single item (${
      ...
      if (pointsToAdd > 0) {
        ...
        console.log(
          `✅ Added ${pointsToAdd} points to user ${order.userId} for order ${orderId}`
        );
