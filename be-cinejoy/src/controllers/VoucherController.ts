...
export default class VoucherController {
  ...
  async addVoucher(req: Request, res: Response): Promise<void> {
    try {
      ...
      const promotionalCode =
        typeof promotionalCodeRaw === "string"
          ? promotionalCodeRaw.trim().toUpperCase()
          : undefined;
      ...
      if (!startDate || !endDate) {
        res
          .status(400)
          .json({ message: "Thiếu startDate/endDate cho voucher" });
        return;
      }
      if (!promotionalCode) {
  ...
  async addPromotionLine(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const updatedVoucher = await voucherService.addPromotionLine(
        id,
        lineData
      );
      if (!updatedVoucher) {
  ...
  async updatePromotionLine(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const updatedVoucher = await voucherService.updatePromotionLine(
        id,
        index,
        lineData
      );
  ...
  async deletePromotionLine(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const updatedVoucher = await voucherService.deletePromotionLine(
        id,
        index
      );
      if (!updatedVoucher) {
  ...
  async redeemVoucher(req: any, res: any) {
    try {
      ...
      const userVoucher = await voucherService.redeemVoucher(userId, {
        voucherId,
        detailId,
      });
      res.json({
  ...
  async getAmountBudgetUsed(req: Request, res: Response): Promise<void> {
    ...
    try {
      if (!id || typeof lineIndex !== "string") {
        res
          ...
          .json({
            status: false,
            error: 1,
            message: "Thiếu id hoặc lineIndex",
            data: null,
          });
        return;
      }
      ...
      if (isNaN(index) || index < 0) {
        res
          ...
          .json({
            status: false,
            error: 1,
            message: "lineIndex không hợp lệ",
            data: null,
          });
        return;
      }
      ...
      res
        .status(200)
        .json({ status: true, error: 0, message: "Thành công", data: result });
    } catch (error: any) {
      res
        .status(500)
        .json({
          status: false,
          error: 1,
          message: error.message || "Lỗi tính ngân sách đã dùng",
          data: null,
        });
    }
  }
