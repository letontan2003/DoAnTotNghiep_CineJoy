    ...
    >
      <div className="max-w-6xl mx-auto px-4 py-8 overflow-x-hidden">
        <div className="flex gap-6 items-start overflow-x-hidden">
          <div
            className={clsx(
              "rounded-2xl p-4 md:p-6 max-w-[820px] mx-auto flex flex-col min-h-[1000px]",
              isDarkMode ? "bg-[#23272f]" : "bg-[#f8f9fa]"
            )}
            style={{ marginLeft: WRAPPER_BLOCK_OFFSET_PX }}
          >
            <h1
              className={clsx(
                "text-lg md:text-xl font-bold mt-1 mb-2",
            ...
            {loading ? (
              ...
              <>
                <div
                  ref={dealsRef}
                  className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-7 gap-y-6 justify-items-start max-w-[776px] mx-auto"
                  style={{ transform: `translateX(${NEWS_GRID_OFFSET_PX}px)` }}
                >
                  {paginated.map((b) => (
                    <div
                      key={b._id}
                      className={clsx(
                        "rounded-xl shadow-md p-3 w-[240px] cursor-pointer hover:scale-[1.02] transition-all duration-200",
                        isDarkMode ? "bg-[#1f2937]" : "bg-white"
                      )}
                      onClick={() => navigate(`/news/${b.blogCode}`)}
                    >
                      ...
                      <div className="mt-3 h-[40px] flex items-center justify-center text-center">
                        <div
                          className={clsx(
                            "font-semibold text-[15px] leading-5 hover:opacity-90 line-clamp-2 block w-full cursor-pointer",
                            isDarkMode ? "text-[#60a5fa]" : "text-[#1e62d0]"
                          )}
                          title={b.title}
                      ...
                      <div className="mt-2 text-center h-5 flex items-center justify-center">
                        <div
                          className="text-[#ff4d4f] text-sm hover:opacity-80 leading-none cursor-pointer"
                          style={{ textDecoration: "none" }}
                        >
                          Xem chi tiết
                        </div>
          ...
          <div className="hidden lg:flex w-[340px] flex-col">
            <div
              className={clsx(
                "rounded-xl shadow p-3 flex flex-col min-h-[1000px]",
                isDarkMode ? "bg-[#23272f]" : "bg-white"
              )}
            >
              <div className="flex justify-center gap-2 mb-3 items-center">
                <button
                  className={`px-5 py-2 w-[160px] whitespace-nowrap text-center rounded cursor-pointer ${
                    activeTab === "now"
              ...
              <div className="space-y-3 overflow-x-hidden">
                {(activeTab === "now"
                  ...
                  <div
                    key={m._id}
                    className={clsx(
                      "flex gap-3 items-center pb-3 last:border-b-0 cursor-pointer hover:brightness-110 transition-all duration-200 rounded p-2",
                      isDarkMode
