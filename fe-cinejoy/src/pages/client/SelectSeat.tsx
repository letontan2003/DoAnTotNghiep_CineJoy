...
export const SelectSeat = () => {
  ...
  const { movie, cinema, date, time, room, showtimeId, theaterId } =
    location.state || {};
  ...
  useLayoutEffect(() => {
    ...
    try {
      ...
      if (raw) {
        console.log(
          "🧹 Clearing sessionStorage but keeping seats reserved on server"
        );
  ...
  useLayoutEffect(() => {
    ...
    try {
      ...
      const perf = performance.getEntriesByType("navigation")[0] as
        ...
        | undefined;
      const isBackForward =
        navigationType === "POP" || (!!perf && perf.type === "back_forward");
      const referrerIsPayment = document.referrer.includes("/payment");
      const hasPaymentFlag =
      ...
      const shouldRestore =
        isBackForward && (hasPaymentFlag || referrerIsPayment);
      ...
      if (!shouldRestore) {
        ...
        console.log(
          "🧹 useLayoutEffect - Clearing sessionStorage - not a valid payment return"
        );
      ...
      if (raw) {
        ...
        if (Array.isArray(restored) && restored.length > 0) {
          console.log(
            "🔄 useLayoutEffect - Restoring selected seats from sessionStorage:",
            restored
          );
          ...
          console.log(
            "🧹 useLayoutEffect - Removed from_payment_page flag after successful restore"
          );
        }
      }
    } catch (error) {
      console.error(
        "Error restoring selected seats in useLayoutEffect:",
        error
      );
  ...
  const validateSeatTypeSelection = useCallback(
    (newSeatType: string): boolean => {
      ...
      const currentTypeName =
        seatTypeNames[selectedSeatType] || selectedSeatType;
  ...
  const handleSelectSeat = useCallback(
    async (seat: string) => {
      ...
      if (isCurrentlySelected) {
        ...
        console.log(
          `🎯 Deselecting seat ${seat} (UI only - will be released when going back from payment)`
        );
      ...
      console.log(
        `🎯 Selected seat ${seat} locally (will reserve on Continue)`
      );
    },
    [
      seatTypeMap,
  ...
  const handleSelectMultipleSeats = useCallback(
    async (seats: string[]) => {
      ...
      if (isCurrentlySelected) {
        ...
        // Ghế sẽ được giải phóng khi user nhấn "Quay về" từ trang Payment
        console.log(
          `🎯 Deselecting couple seats ${seats.join(
            ", "
          )} (UI only - will be released when going back from payment)`
        );
      ...
      console.log(
        `🎯 Selected couple seats ${seats.join(
          ", "
        )} locally (will reserve on Continue)`
      );
    },
    [
      seatTypeMap,
    ...
    try {
      ...
      if (response.status && response.data) {
        ...
        const reservedSeatsData = response.data
          .filter(
            (seat: { status: string; seatId: string }) =>
  ...
  const handleSeatsLoaded = (seatData: {
    ...
    if (seatData?.seats && seatData?.seatLayout) {
      ...
      seatsData.forEach((seatItem: any, index: number) => {
        ...
        const seatId =
          seatItem.seatId ||
          `${String.fromCharCode(65 + fallbackRow)}${fallbackCol + 1}`;
      ...
      console.log(
        "🔍 handleSeatsLoaded - Current selectedSeats:",
        selectedSeats
      );
        ...
        <MovieInfo
          ...
          onContinue={async () => {
            ...
            if (selectedSeats.length > 0) {
              try {
                console.log(
                  "🔒 Reserving seats on Continue (resetting 8-minute timer):",
                  selectedSeats
                );
                ...
                console.log(
                  `✅ Successfully reserved ${selectedSeats.length} seats (timer reset to 8 minutes)`
                );

                // Cập nhật reservedSeats state sau khi reserve thành công
                setReservedSeats((prev) => [
                  ...new Set([...prev, ...selectedSeats]),
                ]);
              } catch (error) {
            ...
            try {
              ...
              console.log(
                "💾 Saved selected seats to sessionStorage:",
                selectedSeats
              );
            } catch (error) {
              console.error(
                "Error saving selected seats to sessionStorage:",
                error
              );
            }
            ...
            navigate("/payment", {
              state: {
                ...
                seats: selectedSeats,
                seatTypeCounts: selectedSeats.reduce(
