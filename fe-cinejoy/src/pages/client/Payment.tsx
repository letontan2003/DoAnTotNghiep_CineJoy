...
const PaymentPage = () => {
  ...
  const availableUserVouchers = useMemo(() => {
    ...
    return userVouchers
      .filter((voucher) => {
        ...
        const endDate =
          (voucher as IUserVoucher)?.voucherId?.validityPeriod?.endDate;
  ...
  const handleApplyVoucher = async (codeOverride?: string) => {
    ...
    const response = await validateVoucherApi(normalizedCode, userId);
      ...
      const applyRes = await applyVoucherApi(normalizedCode, currentSubTotal, userId);
  ...
  useEffect(() => {
    const reapply = async () => {
      ...
      try {
        const applyRes = await applyVoucherApi(appliedVoucher.code, currentSubTotal, userId);
  ...
  return (
                      ...
                      {!appliedVoucher ? (
                        <div className="mb-3 flex flex-col gap-2">
                          ...
                          {voucherCode && !voucherLoading && (
                            <div className={`text-sm text-center font-semibold ${isDarkMode ? "text-cyan-200" : "text-blue-600"}`}>
                              Voucher đã chọn: <span className="uppercase">{voucherCode}</span>
                            </div>
                          )}
    ...
    <Modal
      title={<div className="text-center font-semibold text-base">Chọn voucher</div>}
      ...
      {voucherListLoading ? (
        ...
        >
          {availableUserVouchers.map((voucher) => {
            const voucherDetail = (voucher?.voucherId || {}) as IUserVoucher["voucherId"] & {
              description?: string;
            };
            const title =
              voucherDetail?.description || voucherDetail?.name || "Voucher ưu đãi";
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    >
                      Hạn dùng: {expiry ? formatVoucherExpiry(expiry as string | Date) : "—"}
                    </p>
                    <p
                      className={`text-xs ${
