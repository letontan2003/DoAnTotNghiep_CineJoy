...
const NewsDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadMovies = async () => {
      try {
        ...
        const soon = visibleMovies.filter((m) => {
          const st = normalize(m.status as any);
  ...
  const formatBlogContent = (content: string) => {
    ...
    return lines.map((line, index) => {
      ...
      if (
        trimmedLine.match(/^(Thời gian|Giá bán|Địa điểm|Hotline|Liên hệ).*:/)
      ) {
        return (
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex gap-6">
          <div className="flex-[6]">
            {/* Nút quay lại */}
            <div
              style={{ transform: `translateX(${BACK_BUTTON_OFFSET_PX}px)` }}
            >
              <button
                onClick={() => navigate("/news")}
                className={clsx(
                  "flex items-center gap-2 mb-6 px-4 py-2 rounded-lg hover:opacity-90 transition-all duration-200 cursor-pointer",
                  isDarkMode
            ...
            <div
              ...
              style={{ transform: `translateX(${NEWS_DETAIL_OFFSET_PX}px)` }}
            >
              {/* Nội dung bài viết */}
              <div className="p-6 md:p-8">
                ...
                <div
                  className={clsx(
                    "text-sm mb-6 pb-4 border-b",
                    isDarkMode
                      ? "text-gray-400 border-[#374151]"
                ...
                <div
                  className={clsx(
                    "prose prose-lg max-w-none leading-relaxed blog-content-custom",
            ...
            >
              ...
              <div className="space-y-3">
                {(activeTab === "now"
                  ...
                  : showAllSoon
                  ? moviesSoon
                  : moviesSoon.slice(0, 7)
                ).map((m) => (
                  <div
                    key={m._id}
                    className={clsx(
                      "flex gap-3 items-center pb-3 last:border-b-0 cursor-pointer hover:scale-[1.02] transition-all duration-200 rounded p-2",
                      isDarkMode
                        ? "border-b border-[#374151] hover:bg-[#2a2f38]"
                        : "border-b hover:bg-gray-100"
                    )}
                    onClick={() => navigate(`/movies/${m._id}`)}
                  >
                    <img
                      src={(m.image || m.posterImage) as any}
                      alt={m.title}
                  ...
                  {activeTab === "now"
                    ? moviesNow.length > 7 && (
                        <button
                          ...
                          className="px-4 py-1.5 rounded bg-[#ff6b6b] text-white cursor-pointer hover:bg-[#e05a5a] transition-colors"
                        >
          ...
          <div className="mt-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2
                className={clsx(
                  ...
                  isDarkMode ? "text-[#60a5fa]" : "text-[#1e62d0]"
                )}
              >
                TIN LIÊN QUAN
              </h2>
            ...
            <div className="relative">
              ...
              <button
                ...
                className={clsx(
                  "absolute left-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200",
                  currentSlide === 0
              ...
              <button
                ...
                className={clsx(
                  "absolute right-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200",
                ...
                >
                  {relatedBlogs.map((relatedBlog) => (
                    ...
                    >
                      <div
                        className={clsx(
                          "rounded-xl shadow-md overflow-hidden cursor-pointer hover:scale-[1.02] transition-all duration-200",
                          isDarkMode ? "bg-[#1f2937]" : "bg-white"
                        )}
                        onClick={() =>
                          navigate(`/news/${relatedBlog.blogCode}`)
                        }
                      >
              ...
              <div className="flex justify-center mt-4 gap-2">
                {Array.from({
                  length: Math.max(1, relatedBlogs.length - 3),
                }).map((_, index) => (
                  <button
                    key={index}
