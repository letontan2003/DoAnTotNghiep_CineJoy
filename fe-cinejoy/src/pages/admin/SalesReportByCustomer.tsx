...
const SalesReportByCustomer: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const confirmed = (res.orders || []).filter(
          (o: OrderLite) => o.orderStatus === "CONFIRMED"
        );
  ...
  const rows: RowData[] = useMemo(() => {
    ...
    for (const order of filteredOrders) {
      ...
      const userCode =
        typeof u === "object" ? u?._id || "" : typeof u === "string" ? u : "";
    ...
    for (const key of sortedKeys) {
      ...
      for (const order of ordersOfUser) {
        ...
        const firstType =
          order.seats && order.seats.length > 0 ? order.seats[0].type : "";
        ...
        const concessionInfo = (
          <div>
            {(order.foodCombos || []).map((f, i) => {
              const name =
                typeof f.comboId === "object"
                  ? (f.comboId as { name?: string })?.name
                  : "";
              return (
                <div key={i}>
                  {f.quantity} {name}
        ...
        result.push({
          ...
          concessionText: (order.foodCombos || [])
            .map((f) => {
              const name =
                typeof f.comboId === "object"
                  ? (f.comboId as { name?: string })?.name
                  ...
                  if (r.isGrandTotal) {
                    ...
                    if (
                      ...
                      worksheet.mergeCells(
                        `A${currentGroupStart}:A${excelRow - 1}`
                      );
                    ...
                    const userCodeDisplay = r.userCode
                      ? convertUserIdToTenDigits(r.userCode)
                      : "";
                    row.values = [
                      "",
                      userCodeDisplay,
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              key: "group",
              width: 100,
              render: (_: number, r: RowData) =>
                r.isSubtotal || r.isGrandTotal ? "" : r.stt,
            },
            {
            ...
            },
            {
              ...
              key: "ticketInfo",
              width: 200,
              render: (n: React.ReactNode) => n,
            },
            {
              title: "Concession(s)",
              dataIndex: "concessionInfo",
              key: "concessionInfo",
              width: 220,
              render: (n: React.ReactNode) => n,
            },
