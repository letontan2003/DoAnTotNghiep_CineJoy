...
const ReturnedInvoicePage: React.FC = () => {
  ...
  const printOrder = (order: IOrder) => {
    ...
    const amountDiscountInfo = v?.amountDiscountInfo as
      | {
          description?: string;
          amount?: number;
      ...
      | undefined;
    const amountDiscountVal =
      typeof amountDiscountInfo?.discountValue === "number"
    ...
    const percentPromotions =
      (v?.percentPromotions as Array<{
        description?: string;
        amount?: number;
        discountAmount?: number;
      }>) || [];
    const itemPromotions =
      (v?.itemPromotions as Array<{
        rewardItem?: string;
        rewardQuantity?: number;
        description?: string;
      }>) || [];
    const returnInfo = v?.returnInfo as
      | {
          reason?: string;
          returnDate?: Date;
          refundAmount?: number;
    ...
    const hasPromotions =
      voucher > 0 ||
      amountDiscountVal > 0 ||
      percentPromotions.length > 0 ||
      itemPromotions.length > 0;
    const totalDiscount =
      (voucher || 0) +
      (amountDiscountVal || 0) +
      (percentPromotions.reduce((sum: number, p: any) => {
        const cut =
    ...
    const html = `
      ...
      <body>
        <table class="mb">
          ...
          <tbody>
            <tr>
              ...
              <td>${
                returnInfo?.isBefore2Hours !== undefined
                  ? `Trả vé ${
                      returnInfo.isBefore2Hours ? "trước" : "từ hoặc sau"
                    } 2 tiếng trước giờ chiếu`
                  : "—"
              }</td>
              <td>${
                returnInfo?.refundAmount !== undefined
                  ? `${currency(returnInfo.refundAmount)} (${
                      returnInfo.refundPercentage || 0
                    }%)`
                  : "—"
              }</td>
            </tr>
        ...
        <table>
          ...
          <tbody>
            <tr>
              ...
              <td>
                ${
                  order.foodCombos && order.foodCombos.length
                    ? order.foodCombos
                        .map(
                          (c: any) =>
                            `<div style="margin-bottom: 8px;"><b>${
                              c.comboId?.name || "Combo"
                            }</b><br/>${c.quantity} x ${currency(
                              c.price
                            )}</div>`
                        )
                        .join("")
        ...
        <table class="promo-table">
          <thead>
            <tr>
              ...
              ${
                ...
                amountDiscountVal > 0 ||
                percentPromotions.length > 0
                  ? '<th class="right">Tổng tiền giảm</th>'
                  : ""
              }
            </tr>
          </thead>
          <tbody>
            <tr>
              ${
                amountDiscountVal > 0
                  ? `<td><div style="margin-bottom: 4px; font-size: 13px;">${
                      amountDiscountInfo?.description || ""
                    }</div><div class="discount-amount">- ${currency(
                      amountDiscountVal
                    )}</div></td>`
                  : ""
              }
              ${
                percentPromotions.length
                  ? `<td>${percentPromotions
                      .map((p: any) => {
                        const cut =
                          typeof p.discountAmount === "number"
                            ? p.discountAmount
                            : p.amount || 0;
                        return `<div style="margin-bottom: 8px;"><div style="font-size: 13px;">${
                          p.description || ""
                        }</div><div class="discount-amount">- ${currency(
                          cut
                        )}</div></div>`;
              ...
              ${
                itemPromotions.length
                  ? `<td>${itemPromotions
                      .map(
                        (it: any) =>
                          `<div style="margin-bottom: 8px;"><div style="font-size: 13px;">${
                            it.description || ""
                          }</div><div style="font-weight: bold; color: #16a34a;">+${
                            it.rewardQuantity || 0
                          } ${it.rewardItem || ""}</div></div>`
                      )
                      .join("")}</td>`
                  : ""
              }
              ${
                ...
                amountDiscountVal > 0 ||
                percentPromotions.length > 0
                  ? `<td class="right"><div class="discount-amount" style="font-size: 16px;">- ${currency(
                      totalDiscount
                    )}</div></td>`
                  : ""
              }
        ...
        <div class="total-section">
          <div class="total-amount">Tổng cộng: ${currency(
            order.finalAmount || 0
          )}</div>
        </div>
      </body>
  ...
  const promoInfo = useMemo(() => {
    ...
    const hasVoucher =
      typeof viewOrder.voucherDiscount === "number" &&
  ...
  const rows = useMemo(() => {
    return filtered.map((o) => {
      const returnInfo = (o as any)?.returnInfo as
        | { reason?: string; returnDate?: Date }
        | undefined;
      return {
        id: o._id,
        code: o.orderCode,
        orderDate: dayjs(o.createdAt).format("DD/MM/YYYY"),
        paymentTime: o?.paymentInfo?.paymentDate
          ? dayjs(o.paymentInfo.paymentDate).format("HH:mm DD/MM/YYYY")
          : "—",
        ...
        returnDate: returnInfo?.returnDate
          ? dayjs(returnInfo.returnDate).format("HH:mm DD/MM/YYYY")
          : "—",
        raw: o,
      };
    });
            ...
            <div className="text-black">
              <div className="mb-4 grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded">
                ...
                <div>
                  ...
                  {formatTime((viewOrder as any)?.paymentInfo?.paymentDate)}
                </div>
                <div>
                  ...
                  >
                    {getStatusDisplay(viewOrder.orderStatus || "")}
                  </span>
                </div>
                <div>
                ...
                <div>
                  ...
                  {viewOrder.customerInfo?.fullName || ""}
                </div>
                <div>
                  <strong>Lý do trả:</strong>{" "}
                  {((viewOrder as any)?.returnInfo as { reason?: string })
                    ?.reason || "—"}
                </div>
                <div>
                  <strong>Thời gian trả:</strong>{" "}
                  {formatTime(
                    ((viewOrder as any)?.returnInfo as { returnDate?: Date })
                      ?.returnDate
                  )}
                </div>
                {(
                  (viewOrder as any)?.returnInfo as { isBefore2Hours?: boolean }
                )?.isBefore2Hours !== undefined && (
                  <div>
                    <strong>Thời điểm trả vé:</strong> Trả vé{" "}
                    {(
                      (viewOrder as any)?.returnInfo as {
                        isBefore2Hours?: boolean;
                      }
                    )?.isBefore2Hours
                      ? "trước"
                ...
                {((viewOrder as any)?.returnInfo as { refundAmount?: number })
                  ?.refundAmount !== undefined && (
                  <div>
                    <strong>Số tiền hoàn cho khách:</strong>{" "}
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full border-collapse">
                  ...
                  <tbody>
                    <tr>
                      <td className="p-3 font-medium border border-gray-300 align-top">
                        {(viewOrder.movieId as any)?.title || "—"}
                      </td>
                      <td className="p-3 border border-gray-300 align-top">
                        <div className="mb-1">
                          {(viewOrder.theaterId as any)?.name || "—"}
                        </div>
                        <div className="mb-1">{viewOrder.room}</div>
                        <div className="mb-1">
                          {dayjs(viewOrder.showDate).format("DD/MM/YYYY")}
                        </div>
                        <div>Từ {viewOrder.showTime}</div>
                      </td>
                      <td className="p-3 border border-gray-300 align-top">
                        <div className="mb-1">
                          {viewOrder.seats?.[0]?.type || "VIP"}
                        </div>
                        <div className="mb-1">
                          {viewOrder.seats
                            ?.map((s: any) => s.seatId)
                        ...
                        <div className="font-semibold">
                          {currency(viewOrder.seats?.[0]?.price || 0)}
                        </div>
                      </td>
                      <td className="p-3 border border-gray-300 align-top">
                        {viewOrder.foodCombos &&
                        viewOrder.foodCombos.length > 0 ? (
                          <div className="space-y-2">
                            {viewOrder.foodCombos.map(
                              (combo: any, idx: number) => (
                                <div key={idx} className="mb-2">
                                  ...
                                  <div className="text-sm text-gray-600">
                                    {combo.quantity} x {currency(combo.price)}
                                  </div>
                                </div>
                              )
                            )}
                      ...
                      <td className="p-3 font-semibold border border-gray-300 align-top text-right">
                        {currency(viewOrder.totalAmount || 0)}
                      </td>
                    </tr>
                  </tbody>
                </table>
                          ...
                          {promoInfo?.hasAmountDiscount && (
                            <td className="p-3 border border-gray-300 align-top">
                              <div className="mb-1 text-sm">
                                {promoInfo?.amountDiscountInfo?.description}
                              </div>
                              <div className="font-bold text-black">
                                - {currency(promoInfo?.amountDiscountVal || 0)}
                              </div>
                            </td>
                          )}
                          {promoInfo?.hasPercentPromotions && (
                            <td className="p-3 border border-gray-300 align-top">
                              {promoInfo?.percentPromotions.map(
                                (p: any, i: number) => {
                                  const cut =
                                    typeof p?.discountAmount === "number"
                                      ? p.discountAmount
                              ...
                              <div className="text-lg font-bold text-black">
                                ...
                                {currency(
                                  (viewOrder.voucherDiscount || 0) +
                                    ...
                                    (promoInfo?.percentPromotions?.reduce(
                                      (sum: number, p: any) => {
                                        const cut =
                                          typeof p?.discountAmount === "number"
                                            ? p.discountAmount
              ...
              <div className="mt-6 pt-4 border-t-2 border-gray-400 text-right">
                <div className="text-xl font-bold">
                  ...
                  <span className="text-red-600">
                    {currency(viewOrder.finalAmount || 0)}
                  </span>
                </div>
              </div>
            </div>
