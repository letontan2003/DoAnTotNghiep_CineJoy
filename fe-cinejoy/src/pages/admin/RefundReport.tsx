...
const RefundReport: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const returned = (res.orders || []).filter((o: OrderLite) => o.orderStatus === 'RETURNED');
  ...
  const exportExcel = async () => {
    ...
    const headers = [
      'STT', 'Mã hóa đơn', 'Tên khách hàng', 'Ngày đặt', 'Thời gian thanh toán', 'Phương thức thanh toán',
      'Thời gian trả', 'Lý do trả', 'Sản Phẩm', 'Suất chiếu', 'Vé', 'Concession',
      'Thành tiền', 'Tổng cộng', 'Doanh số trước CK', 'Chiết khấu', 'Doanh số sau CK'
    ];
    ...
    rows.forEach((r, i) => {
      ...
      if (r.isGrandTotal) {
        row.values = ['Tổng cộng', '', '', '', '', '', '', '', '', '', '', '', fmt(r.totalAmount), fmt(r.finalAmount), fmt(r.revenueBeforeDiscount), fmt(r.discount), fmt(r.revenueAfterDiscount)];
        row.font = { bold: true };
        ...
        row.values = [
          '', r.orderCode, r.customerName, r.orderDate, r.paymentTime, r.paymentMethod,
          r.returnTime, r.returnReason, r.product, r.showtime, r.tickets, r.concessions,
          fmt(r.totalAmount), fmt(r.finalAmount), fmt(r.revenueBeforeDiscount), fmt(r.discount), fmt(r.revenueAfterDiscount)
        ];
    ...
    worksheet.columns = [
      { width: 10 }, { width: 18 }, { width: 20 }, { width: 12 }, { width: 18 }, { width: 18 },
      { width: 18 }, { width: 25 }, { width: 25 }, { width: 35 }, { width: 20 }, { width: 25 },
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'orderDate',
      width: 120,
      render: (text: string, r: RowData) => (r.isSubtotal || r.isGrandTotal) ? '' : text,
    },
    {
    ...
    },
    {
      ...
      render: (value: number, r: RowData) => {
        if (r.isGrandTotal) return <span style={{ color: 'red', fontWeight: 700 }}>{value.toLocaleString('vi-VN')}</span>;
