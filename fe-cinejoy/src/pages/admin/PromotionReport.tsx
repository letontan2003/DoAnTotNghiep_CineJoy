    ...
    try {
      ...
      for (const voucher of vouchers) {
        ...
        for (let i = 0; i < voucher.lines.length; i++) {
          ...
          const reportItem: PromotionReportData = {
            ...
            startDate: dayjs(line.validityPeriod.startDate).format(
              "DD/MM/YYYY"
            ),
          ...
          if (line.promotionType === "item") {
            ...
            reportItem.giftQuantity =
              detail?.totalQuantity || detail?.quantity || 0;
            reportItem.unit = "Phiếu";
            reportItem.discountValue = detail?.discountPercent || 0;
          } else if (line.promotionType === "percent") {
            reportItem.unit = "phần trăm";
            reportItem.discountValue =
              detail?.comboDiscountPercent ||
              detail?.ticketDiscountPercent ||
              0;
      ...
      if (allPromotions.length > 0) {
        ...
        const minStart = dates.reduce(
          (min, d) => (d.start.isBefore(min) ? d.start : min),
          dates[0].start
        );
        const maxEnd = dates.reduce(
          (max, d) => (d.end.isAfter(max) ? d.end : max),
          dates[0].end
        );
        setMinDate(minStart);
        setMaxDate(maxEnd);
  ...
  useEffect(() => {
    ...
    if (dateRange) {
      ...
      filtered = filtered.filter((item) => {
        ...
        return (
          itemStart.isSameOrAfter(startDate) && itemEnd.isSameOrBefore(endDate)
        );
      });
    }
  ...
  const exportToExcel = async () => {
    ...
    if (dateRange && dateRange[0] && dateRange[1]) {
      ...
      if (allDates.length > 0) {
        const minStart = allDates.reduce(
          (min, d) => (d.start.isBefore(min) ? d.start : min),
          allDates[0].start
        );
        const maxEnd = allDates.reduce(
          (max, d) => (d.end.isAfter(max) ? d.end : max),
          allDates[0].end
        );
    ...
    worksheet.getCell(
      "A4"
    ).value = `Từ ngày: ${fromDate}         Đến ngày: ${toDate}`;
    ...
    filteredData.forEach((item, index) => {
      ...
      row.values = [
        ...
        item.promotionType === "voucher"
          ? "Voucher"
          : item.promotionType === "item"
          ? "Khuyến mãi hàng"
          : item.promotionType === "percent"
          ? "Khuyến mãi chiết khấu"
          : item.promotionType === "amount"
          ? "Khuyến mãi tiền"
          : item.promotionType,
        item.name,
        item.status,
