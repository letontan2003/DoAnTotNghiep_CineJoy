...
const VoucherForm: React.FC<VoucherFormProps> = ({
    ...
    if (voucher) {
      form.setFieldsValue({
        ...
        startDate: voucher.startDate
          ...
          ? dayjs(voucher.validityPeriod.startDate)
          : undefined,
        endDate: voucher.endDate
          ? dayjs(voucher.endDate)
          : voucher.validityPeriod?.endDate
          ? dayjs(voucher.validityPeriod.endDate)
          : undefined,
  ...
  const validateStartDateWithLines = (
    ...
    if (startDate.isAfter(minLineStart.startOf("day"))) {
      return {
        isValid: false,
        message: `Phải trước hoặc bằng ngày bắt đầu sớm nhất: ${minLineStart.format(
          "DD/MM/YYYY"
        )}`,
      };
    }
  ...
  const validateEndDateWithLines = (
    endDate: dayjs.Dayjs
  ): { isValid: boolean; message?: string } => {
    if (!voucher || !voucher.lines || voucher.lines.length === 0) {
      return { isValid: true };
    ...
    if (endDate.isBefore(maxLineEnd.startOf("day"))) {
      return {
        isValid: false,
        message: `Phải sau hoặc bằng ngày kết thúc trễ nhất: ${maxLineEnd.format(
          "DD/MM/YYYY"
        )}`,
      };
    }
  ...
  const isDateInOverlapRange = (date: dayjs.Dayjs): boolean => {
    return existingVouchers.some((existingVoucher) => {
      ...
      return (
        date.isSameOrAfter(existingStart) && date.isSameOrBefore(existingEnd)
      );
    });
  };
    ...
    <Modal
      ...
      title={
        <div className="text-center text-xl md:text-xl font-semibold">
          {voucher ? "Sửa khuyến mãi" : "Thêm khuyến mãi mới"}
        </div>
      }
      onCancel={onCancel}
      footer={null}
              ...
              validator: (_, value) => {
                ...
                if (existingVoucher) {
                  return Promise.reject(
                    new Error("Mã khuyến mãi này đã tồn tại!")
                  );
                }
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    <Select
                      ...
                      options={[{ value: "percent", label: "Phần trăm (%)" }]}
                      disabled
                    />
                  </Form.Item>
                ...
                >
                  {({ getFieldValue }) => {
                    const applyType = getFieldValue("applyType");
                    if (
                      (applyType === "ticket" || applyType === "combo") &&
                      !showDetails
                    ) {
                      return (
                    ...
                    if (
                      ...
                      return (
                        <div className="space-y-3 border border-gray-200 rounded-lg p-3 bg-gray-50">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <Form.Item
                              shouldUpdate={(prev, cur) =>
                                prev.comboName !== cur.comboName ||
                                prev.seatType !== cur.seatType
                              }
                            >
                              {({ getFieldValue }) => (
                                <Form.Item
                                ...
                                >
                                  {applyType === "combo" ? (
                                    <Select
                                      placeholder={
                                        getFieldValue("comboName")
                                          ? "Chọn combo"
                                          : "Chọn Tên combo trước"
                                      }
                                      size="large"
                                      showSearch
                                      optionFilterProp="children"
                                      ...
                                      onChange={(value, option: any) => {
                                        ...
                                        const selectedComboName =
                                          getFieldValue("comboName");
                                        if (!selectedComboName) {
                                          message.warning(
                                            "Vui lòng chọn Tên combo trước."
                                          );
                                        ...
                                        if (value !== selectedComboName) {
                                          message.error(
                                            "Mua combo phải trùng với Tên combo đã chọn."
                                          );
                                          form.setFieldValue(
                                            "buyItem",
                                            selectedComboName
                                    ...
                                    <Select
                                      ...
                                      onChange={(value) => {
                                        ...
                                        const expected = seatType
                                          ? labelMap[seatType as string]
                                          : undefined;
                                        if (!seatType) {
                                          message.warning(
                                            "Vui lòng chọn Loại ghế trước."
                                          );
                                          form.setFieldValue(
                                            "buyItem",
                                            undefined
                                        ...
                                        if (expected && value !== expected) {
                                          message.error(
                                            "Loại vé phải trùng với Loại ghế đã chọn."
                                          );
                                          form.setFieldValue(
                                            "buyItem",
                                            expected
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <Form.Item name="rewardItem" label="Sản phẩm tặng">
                              <Select
                          ...
                          <Form.Item name="rewardType" label="Loại tặng">
                            <Select
                          ...
                          <Form.Item
                            shouldUpdate={(prev, cur) =>
                              prev.rewardType !== cur.rewardType
                            }
                          >
                            {({ getFieldValue }) => {
