...
const TheaterForm: React.FC<TheaterFormProps> = ({
  ...
  useEffect(() => {
    if (theater) {
      ...
      if (matchingRegion) {
        console.log(
          `Found matching region for theater "${theater.name}":`,
          matchingRegion.name
        );
      } else {
        console.warn(
          `No matching region found for theater "${theater.name}" with city "${theater.location.city}"`
        );
        console.log(
          "Available regions:",
  ...
  const handleSubmit = async (values: {
    ...
    const selectedRegion = regions.find((region) => {
      ...
      if (regionName.includes(cityName) || cityName.includes(regionName))
        return true;
    ...
    if (!selectedRegion) {
      ...
      toast.error(
        `Không tìm thấy khu vực tương ứng với "${values.location.city}"! Vui lòng kiểm tra lại dữ liệu khu vực.`
      );
      return;
    }
        ...
        <div className="grid grid-cols-2 gap-4">
          <Form.Item
            ...
            rules={[
              ...
              {
                pattern: /^RC\d{3}$/,
                message: "Mã rạp phải có định dạng RC001, RC002, ...",
              },
              {
                validator: (_, value) => {
                  ...
                  const existingTheater = theaters.find(
                    (t) =>
                      t.theaterCode === value &&
          ...
          >
            <Input placeholder="RC001, RC002, ..." size="large" />
          </Form.Item>
