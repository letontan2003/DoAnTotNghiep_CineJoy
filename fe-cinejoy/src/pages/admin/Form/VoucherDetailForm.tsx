...
const VoucherDetailForm: React.FC<VoucherDetailFormProps> = ({
  ...
  const [stackingPolicy, setStackingPolicy] = useState<string | undefined>(
    undefined
  );
  ...
  const isStatusOnlyEditable = Boolean(
    editingLine && initialStatus === "hoạt động"
  );
  ...
  useEffect(() => {
    if (editingLine) {
      ...
      form.setFieldsValue({
        ...
        stackingPolicy:
          editingLine.rule?.stackingPolicy ||
          (editingLine.promotionType === "voucher"
            ? "EXCLUSIVE"
            : "EXCLUSIVE_WITH_GROUP"),
        exclusionGroup:
          editingLine.rule?.exclusionGroup ||
          (editingLine.promotionType === "amount"
            ? "order-discount"
            : undefined),
        ...
        rewardType:
          detail?.rewardType ||
          (editingLine.promotionType === "item" ? "free" : undefined),
  ...
  const handleSubmit = async () => {
    try {
      ...
      const rule = {
        stackingPolicy: values.stackingPolicy as
          | "STACKABLE"
          | "EXCLUSIVE"
          | "EXCLUSIVE_WITH_GROUP",
    ...
    <Modal
      title={
        <div className="text-center text-xl md:text-xl font-semibold">
          {editingLine ? "Sửa chi tiết khuyến mãi" : "Thêm chi tiết khuyến mãi"}
        </div>
      }
      open={visible}
      ...
      >
        <Form form={form} layout="vertical" initialValues={{}}>
          <Form.Item
            ...
            rules={[
              { required: true, message: "Vui lòng chọn kiểu khuyến mãi!" },
            ]}
          >
            <Select
            ...
            rules={[
              ...
              () => ({
                validator(_, value) {
                  ...
                  if (
                    selectedDate.isBefore(voucherStart.startOf("day")) ||
                    selectedDate.isAfter(voucherEnd.endOf("day"))
                  ) {
                    return Promise.reject(
                      new Error(
                        `Ngày bắt đầu phải nằm trong khoảng ${voucherStart.format(
                          "DD/MM/YYYY"
                        )} - ${voucherEnd.format("DD/MM/YYYY")}`
                      )
                    );
                  }
                  return Promise.resolve();
          ...
          >
            <DatePicker
              ...
              disabledDate={(current) => {
                ...
                const minStart = voucherStart.isAfter(today)
                  ? voucherStart
                  : today;
                return (
                  current.isBefore(minStart) || current.isAfter(voucherEnd)
                );
              }}
            />
          </Form.Item>
              ...
              () => ({
                validator(_, value) {
                  ...
                  if (
                    ...
                    return Promise.reject(
                      new Error(
                        `Ngày kết thúc phải nằm trong khoảng ${voucherStart.format(
                          "DD/MM/YYYY"
                        )} - ${voucherEnd.format("DD/MM/YYYY")}`
                      )
                    );
          ...
          >
            <DatePicker
              ...
              disabledDate={(current) => {
                if (!current || !voucherStartDate || !voucherEndDate)
                  return false;
                const voucherStart = dayjs(voucherStartDate);
                const voucherEnd = dayjs(voucherEndDate);
                ...
                if (
                  current.isBefore(voucherStart.startOf("day")) ||
                  current.isAfter(voucherEnd.endOf("day"))
                ) {
                  return true;
                }
                ...
                if (
                  startDate &&
                  current.isBefore(dayjs(startDate).startOf("day"))
                ) {
                  return true;
                }
            ...
            rules={[
              { required: true, message: "Vui lòng chọn trạng thái!" },
              {
                validator: (_, value) => {
                  ...
                  if (
                    voucherStatus === "không hoạt động" &&
                    value === "hoạt động"
                  ) {
                    return Promise.reject(
                      new Error(
                        'Không thể đặt trạng thái "hoạt động" khi voucher header đang "không hoạt động"'
                      )
                    );
                  }
                  return Promise.resolve();
          ...
          {(promotionType === "amount" ||
            ...
            <>
              ...
              <Form.Item
                ...
                hidden
                initialValue={
                  promotionType === "amount" ? "order-discount" : undefined
                }
              >
                <input type="hidden" />
              </Form.Item>
              ...
              <>
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Vui lòng chọn quy tắc áp dụng!",
                    },
                  ]}
                >
                  <Select
                    ...
                    disabled={
                      isStatusOnlyEditable || promotionType === "voucher"
                    }
                  ...
                  >
                    ...
                    <Option value="EXCLUSIVE_WITH_GROUP">
                      Loại trừ theo nhóm
                    </Option>
                  </Select>
                </Form.Item>
                ...
                {stackingPolicy === "EXCLUSIVE_WITH_GROUP" && (
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: "Vui lòng chọn hoặc nhập nhóm loại trừ!",
                      },
                    ]}
                  >
                    <Select
                      ...
                      options={getExclusionGroups(promotionType || "").map(
          ...
          {promotionType && promotionType === "voucher" && (
            <>
              <div className="border-t pt-4 mt-4">
                ...
                <Form.Item name="voucherDescription" label="Mô tả">
                  <Input.TextArea
                    disabled={isStatusOnlyEditable}
                    rows={3}
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-8">
                  <Form.Item
                    ...
                    label="Điểm đổi"
                    rules={[
                      { required: true, message: "Vui lòng nhập điểm đổi!" },
                    ]}
                  >
                    <InputNumber
                      disabled={isStatusOnlyEditable}
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-8">
                  ...
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: "Vui lòng nhập ngân sách tổng!",
                      },
                    ]}
                  >
                    <InputNumber
              ...
              <div className="border-t pt-4 mt-4">
                ...
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Vui lòng nhập mô tả khuyến mãi hàng!",
                    },
                    { max: 200, message: "Mô tả không được quá 200 ký tự!" },
                  ]}
                >
                ...
                <Form.Item
                  shouldUpdate={(prevValues, currentValues) =>
                    prevValues.applyType !== currentValues.applyType
                  }
                >
                  {({ getFieldValue }) => {
                              ...
                              },
                              {
                                validator: (_, value) => {
                                  ...
                                  if (value && !comboId) {
                                    return Promise.reject(
                                      new Error(
                                        "Vui lòng chọn lại combo để lấy ID!"
                                      )
                                    );
                                  }
                                  return Promise.resolve();
                              ...
                              onChange={(value, option: any) => {
                                if (option && option.data) {
                                  ...
                                  if (promotionType === "item") {
                                    ...
                                    console.log(
                                      "Auto-generated exclusionGroup for combo:",
                                      exclusionGroup
                                    );
                                  }
                                }
                          ...
                          <Select
                            ...
                            onChange={(value) => {
                              ...
                              if (promotionType === "item") {
                                ...
                                console.log(
                                  "Auto-generated exclusionGroup for ticket:",
                                  exclusionGroup
                                );
                              }
                            }}
                        ...
                        <>
                          ...
                          <Form.Item
                            ...
                            rules={[
                              ...
                              },
                              {
                                validator: (_, value) => {
                                  const rewardItemId =
                                    form.getFieldValue("rewardItemId");
                                  if (value && !rewardItemId) {
                                    return Promise.reject(
                                      new Error(
                                        "Vui lòng chọn lại sản phẩm tặng để lấy ID!"
                                      )
                                    );
                                  }
                                  return Promise.resolve();
                          ...
                          <Form.Item
                            ...
                            rules={[
                              {
                                required: true,
                                message: "Vui lòng nhập số lượng tặng!",
                              },
                            ]}
                          >
                            <InputNumber
          ...
          {promotionType && promotionType === "percent" && (
            <>
              <div className="border-t pt-4 mt-4">
                <h4 className="text-lg font-semibold mb-4">
                  Chi tiết khuyến mãi chiết khấu
                </h4>
