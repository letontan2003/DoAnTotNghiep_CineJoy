...
const FoodComboForm: React.FC<FoodComboFormProps> = ({
  combo,
  onSubmit,
  onCancel,
}) => {
  ...
  useEffect(() => {
    if (combo) {
      ...
      const processedItems = (combo.items || []).map(
        (item: IComboItem & { productId: string | { _id: string } }) => ({
          ...item,
          productId:
  ...
  const handleSubmit = async (
    values: Partial<IFoodCombo> & { code?: string }
  ) => {
    try {
      setIsLoading(true);
  ...
  return (
    ...
    >
      ...
      >
          ...
          rules={[
            ...
            // Kiểm tra trùng mã tại FE
            {
              validator: async (_: unknown, value: string) => {
                ...
                try {
                  const { getFoodCombos } = await import(
                    "@/apiservice/apiFoodCombo"
                  );
                  const all = await getFoodCombos();
                  const currentId = (combo as any)?._id;
                  const exists = all.some(
                    (c: any) =>
                      String(c.code).toUpperCase() === v.toUpperCase() &&
                      c._id !== currentId
                  );
                  if (exists)
                    return Promise.reject(
                      new Error("Mã đã tồn tại, vui lòng chọn mã khác")
                    );
                } catch {}
            ...
            },
            {
              validator: (_, value) => {
                ...
                const ok =
                  productType === "single" ? /^SP\d{3,}$/ : /^CB\d{3,}$/;
                return ok.test(v)
                  ? Promise.resolve()
                  : Promise.reject(
                      new Error(
                        productType === "single"
                          ? "Định dạng SPxxx, ví dụ SP001"
                          : "Định dạng CBxxx, ví dụ CB001"
                      )
                    );
              },
            },
        ...
        >
          <Input
            placeholder={productType === "single" ? "VD: SP001" : "VD: CB001"}
            size="large"
          />
        </Form.Item>
          ...
          rules={[
            {
              ...
              message: `Vui lòng nhập tên ${
                productType === "single" ? "sản phẩm" : "combo"
              }!`,
            },
            {
              min: 3,
          ...
          <>
            ...
            <Form.Item
              ...
              shouldUpdate={(prevValues, currentValues) => {
                ...
                const hasProductIdChange = currentItems.some(
                  (item: any, index: number) => {
                    const prevItem = prevItems[index];
                    if (!prevItem && item) return true; // Item mới được thêm
                    const prevProductId =
                      typeof prevItem?.productId === "object"
                        ...
                        {fields.map(({ key, name, ...restField }) => {
                          ...
                          const filteredProducts = availableProducts.filter(
                            (product) => {
                              ...
                              const isSelectedInOtherField = items.some(
                                (item: any, index: number) => {
