...
const Seat: React.FC<SeatProps> = ({
  ...
  const [seatMap, setSeatMap] = useState<
    Record<
      string,
      { type: SeatType; status: SeatStatus; isReservedByMe?: boolean }
    >
  >({});
  ...
  const handleSeatSelection = (seatName: string) => {
    ...
    if (seatType === "couple") {
      ...
      const isPairSeatReservedByMe =
        seatMap[pairSeatName]?.isReservedByMe || false;

      const isSelectedFromServer =
        (seatStatus === "selected" && !isSeatReservedByMe) ||
        (seatStatus === "reserved" && !isSeatReservedByMe) ||
        seatStatus === "occupied" ||
        seatStatus === "keepwaiting" ||
        soldSeats.includes(seatName) ||
        (pairSeatStatus === "selected" && !isPairSeatReservedByMe) ||
        (pairSeatStatus === "reserved" && !isPairSeatReservedByMe) ||
        pairSeatStatus === "occupied" ||
        pairSeatStatus === "keepwaiting" ||
        soldSeats.includes(pairSeatName);
      ...
      if (!isSelectedFromServer && !isMaintenance && !isPairMaintenance) {
        ...
        const isPairSelected =
          selectedSeats.includes(pairSeatName) ||
          (pairSeatStatus === "selected" && isPairSeatReservedByMe);
    ...
    const loadSeats = async () => {
      ...
      try {
        ...
        if (response.status && response.data) {
          ...
          const selectedSeats = Object.entries(map).filter(
            ([_, info]) => info.status === "selected"
          );
          ...
          try {
            ...
            if (token) {
              ...
              const isFromPaymentReturn =
                sessionStorage.getItem("from_payment_page") === "true";
              ...
              if (reservationResponse.status && reservationResponse.data) {
                ...
                reservationResponse.data.forEach((seatItem: any) => {
                  if (seatItem.seatId && updatedMap[seatItem.seatId]) {
                    console.log(
                      `🔍 Updating seat ${seatItem.seatId}: status=${seatItem.status}, isReservedByMe=${seatItem.isReservedByMe}`
                    );
                    ...
                    if (seatItem.status === "reserved") {
                      if (seatItem.isReservedByMe) {
                        ...
                        console.log(
                          `✅ Seat ${seatItem.seatId} set to 'selected' (reserved by me)`
                        );
                      } else {
                        ...
                        console.log(
                          `🔒 Seat ${seatItem.seatId} set to 'reserved' (by others)`
                        );
                      }
                    ...
                    } else if (seatItem.status === "keepwaiting") {
                      // Keepwaiting seats - giữ nguyên status để hiển thị đúng màu
                      updatedMap[seatItem.seatId].status = "keepwaiting";
                      updatedMap[seatItem.seatId].isReservedByMe =
                        seatItem.isReservedByMe || false;
                      console.log(
                        `⏳ Seat ${seatItem.seatId} set to 'keepwaiting' (isReservedByMe: ${seatItem.isReservedByMe})`
                      );
                      ...
                      updatedMap[seatItem.seatId].isReservedByMe = false;
                      console.log(
                        `🟢 Seat ${seatItem.seatId} set to 'available' (released)`
                      );
                    }
                  }
                });
                ...
                console.log(
                  "🪑 Updated seat map with reservations:",
                  updatedMap
                );
              }
            }
            ...
            console.log(
              "ℹ️ Could not load reservation status (expected if no auth):",
              reservationError
            );
          }
        } else {
  ...
  const renderRows = seatLayout
    ? Array.from({ length: seatLayout.rows }, (_, i) =>
                ...
                <div key={`pair-${pairIndex}`} className="flex gap-1">
                  {[0, 1].map((seatInPair) => {
                    ...
                    const isReservedByMe =
                      seatMap[seatName]?.isReservedByMe || false;
                    const isMaintenance = seatStatus === "maintenance";
                    const isSelectedFromServer =
                      (seatStatus === "selected" && !isReservedByMe) ||
                      (seatStatus === "reserved" && !isReservedByMe) ||
                      seatStatus === "occupied" ||
                      seatStatus === "keepwaiting" ||
                      soldSeats.includes(seatName);
                    ...
                    const colorClass = isMaintenance
                      ...
                      : seatStatus === "keepwaiting"
                      ? "bg-[#b3210e] border-[#b3210e] text-white" // Ghế keepwaiting luôn hiển thị đỏ (giống occupied)
                      : seatStatus === "reserved"
                      ? isReservedByMe
                        ? "bg-blue-600 border-blue-600 text-white" // Ghế reserved của user hiện tại = xanh (selected)
                        : "bg-[#b3210e] border-[#b3210e] text-white" // Ghế reserved của user khác = đỏ
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-6.5 h-6.5 ${colorClass} border-2 rounded flex items-center justify-center text-[10px] font-bold relative ${
                            isSelectedFromServer ? "cursor-not-allowed" : ""
                          }`}
                        >
                          {seatName}
                          {isMaintenance && (
