  ...
  return (
    <div className="w-full">
      <div className="flex justify-center gap-4 mb-8 pt-3 pb-3">
        <button
          className={`px-6 h-10 border rounded font-semibold transition cursor-pointer ${
            activeTab === "Phim sắp chiếu"
              ...
              : `${
                  isDarkMode
                    ? "bg-[#3a3c4a] text-gray-200 border-gray-600 hover:bg-blue-700 hover:border-blue-700 hover:text-white"
      ...
      {filteredMovies.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-y-6 px-4">
          {(showMore ? filteredMovies : filteredMovies.slice(0, 7)).map(
            (movie) => (
              <div
                key={movie._id}
                className={`${
                  isDarkMode
                    ? "bg-[#282a36] text-gray-200 border-gray-700 shadow-lg"
                    : "bg-white text-[#2d3a5a] border shadow-md"
                } rounded-xl overflow-hidden w-[270px] mx-auto flex flex-col items-center`}
              >
                ...
                <div className="p-4 w-full flex flex-col items-center flex-1">
                  <h3
                    className={`${
                      isDarkMode ? "text-white" : "text-[#2d3a5a]"
                    } text-base font-semibold text-center leading-tight mb-1`}
                  >
                    {movie.title}
                  </h3>
                  {movie.actors && (
                    <p
                      ...
                      } text-center text-sm mb-1 line-clamp-2 h-10`}
                    >
                      Diễn viên:{" "}
                  ...
                  <button
                    className={`${
                      isDarkMode
                        ...
                        : "bg-[#162d5a] hover:bg-[#1a376e] text-white"
                    } w-full mt-2 py-2 rounded font-semibold transition cursor-pointer`}
      ...
      {filteredMovies.length > 7 && (
        <div className="flex justify-center mt-4">
          <button
            className={`${
              isDarkMode
                ? "px-6 h-8 border rounded text-gray-200 border-gray-600 bg-[#3a3c4a] hover:bg-blue-700 hover:border-blue-700 hover:text-white"
                : "px-6 h-8 border rounded bg-white text-[#2d3a5a] hover:bg-[#2d3a5a] hover:text-white"
