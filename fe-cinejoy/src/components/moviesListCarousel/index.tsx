...
const MoviesListCarousel = (props: IProps) => {
  ...
  const [currentMovieForModal, setCurrentMovieForModal] =
      ...
      className={`${
        props.bg
          ? isDarkMode
            ? "bg-[#23272f]"
            : "bg-[#b3c3a726]"
          : "bg-[url('https://vticinema.web.app/assets/notification_bg-B6yeZWl8.jpg')]"
      } py-16 px-8`}
    >
        ...
        <h2
          className={`${
            props.titleColor
              ...
              : "text-white"
          } text-3xl font-medium mb-10 text-center`}
        >
          {props.title}
        </h2>
            ...
            .map((movie, index) => {
              return (
                <Link to={`/movies/${movie._id}`} key={movie._id}>
                  <div className="relative group cursor-pointer overflow-hidden rounded-md transition-all duration-300 hover:scale-110 hover:z-999">
                    {/* Number Badge */}
                    ...
                    {/* Age Rating Badge */}
                    <div
                      className={`absolute top-4 right-4 ${getRatingBadgeColor(
                        movie.ageRating
                      )} text-white px-2 py-1 rounded font-bold z-1`}
                    >
                      {movie.ageRating}
                    </div>
                    ...
                    <div className="relative aspect-[2/3]">
                      ...
                      <div
                        className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-center justify-center group-hover:opacity-100 transition-opacity duration-300 ${
                          movie.trailer ? "opacity-0" : "opacity-0"
                        }`}
                        onClick={(e) => {
                          e.stopPropagation();
                          e.preventDefault();
            ...
            <div className="mt-4 flex items-start">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-xl font-bold text-[#0f1b4c] mb-1">
                    {currentMovieForModal.title}
                  </h3>
                ...
                <p className="text-gray-700 text-sm line-clamp-3">
                  {currentMovieForModal.description}
                </p>
              </div>
            </div>
